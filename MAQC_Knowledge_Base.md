...
```mermaid
graph TD
    A[Ping Opponent in Battle Post] --> B{Respond within 72 hrs?}
    B -- Yes --> C[Schedule Match]
    B -- No --> D{Is it before Friday 6AM UTC?}
    D -- Yes --> E[Open Ticket in #maqc-support-leaders]
    D -- No --> F[Both Teams Marked Unresponsive]
    E --> G[Moderator Review]
    G --> H[Possible Auto-Win Awarded]
    F --> I[Possible Disqualification]
```
...
graph TD
    ...
    B -- Disconnect/Crash --> C[Take Screenshot Immediately]
    B -- SP Violation --> D[Gather Proof, Do Not Engage]
    B -- Misconduct --> E[Do Not Engage, Gather Proof]
    C --> F[Open Ticket in #maqc-support-leaders with Match Number]
    D --> F
    E --> F
    F --> G[Moderator Investigation]
    G --> H[Ruling Final & Non-Negotiable]
```
